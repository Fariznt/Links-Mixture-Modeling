...
generated quantities {
  array[N] int<lower=1, upper=2> z; // Mixture membership for each observation
  for (n in 1 : N) {
    vector[2] log_weights;
    log_weights[1] = log(theta)
                     + bernoulli_logit_lpmf(y[n] | dot_product(X[n], beta1));
    log_weights[2] = log1m(theta)
                     + bernoulli_logit_lpmf(y[n] | dot_product(X[n], beta2));
